...
class Particle {
  ...
  draw() {
    ctx.fillStyle = 'rgba(77, 230, 6, 0.8)' // Color verde neón
    ctx.beginPath()
...
const drawLines = () => {
  for (let a = 0; a < particles.length; a++) {
    for (let b = a; b < particles.length; b++) {
      ...
      if (distance < 100) {
        ctx.strokeStyle = `rgba(77, 230, 6, ${1 - distance / 100})`
        ctx.lineWidth = 0.5
        ctx.beginPath()
...
.background-canvas {
  ...
  background: radial-gradient(circle at center, #ffff00 0%, #000000 100%);
}
</style>